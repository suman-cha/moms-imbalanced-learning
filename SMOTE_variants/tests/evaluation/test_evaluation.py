...
from sm_variants.evaluation import Folding, SamplingJob, EvaluationJob
import sm_variants
...
def test_evaluation():
    ...
    folding = Folding(DATASET, cache_path=CACHE_PATH, reset=True)
    ...
    for fold in folding.fold():
        ...
        ejob = EvaluationJob(result, CLASSIFIERS, cache_path=CACHE_PATH)
...
def test_evaluation_warning():
    ...
    """
    folding = Folding(DATASET, reset=True)

    sm_variants.config.suppress_internal_warnings(False)

    for fold in folding.fold():
        sjob = SamplingJob(fold, OVERSAMPLER, OVERSAMPLER_PARAMS)
        ...
        sm_variants.config.suppress_internal_warnings(True)
...
def test_sampling_no_caching():
    ...
    for fold in folding.fold():
        ...
        ejob = EvaluationJob(result, CLASSIFIERS)
...
def test_get_params():
    ...
    """
    ejob = EvaluationJob({}, CLASSIFIERS, cache_path=CACHE_PATH)
