...
from sm_variants.evaluation import Folding
...
def test_basic():
    ...
    """
    folding = Folding(dataset, cache_path=cache_path, serialization="json")
...
def test_json_specific_validator():
    ...
    folding = Folding(
        dataset,
        validator_params={"n_repeats": 1, "n_splits": 5, "random_state": 5},
    ...
    folding = Folding(dataset, cache_path=cache_path, serialization="json", reset=True)
    lens = [len(fold["X_test"]) for fold in folding.fold()]
    assert len(lens) == 10
...
def test_pickle():
    ...
    """
    folding = Folding(dataset, cache_path=cache_path, serialization="pickle")
    lens = [len(fold["X_test"]) for fold in folding.fold()]
    assert len(lens) == 10
    assert len(folding.folding_files()) == 10
...
def test_nonsense():
    ...
    with pytest.raises(ValueError):
        Folding(
            dataset, cache_path=cache_path, serialization="nonsense"
        ).cache_foldings()
