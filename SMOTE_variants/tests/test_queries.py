...
def test_queries():
    ...
    assert len(sv.get_metric_learning_oversamplers(n_quickest=2)) == 2
    assert len(sv.get_simplex_sampling_oversamplers()) >= 0
    assert len(sv.get_simplex_sampling_oversamplers(n_quickest=2)) == 2
    assert (
        len(sv.get_simplex_sampling_oversamplers(within_simplex_sampling="random")) >= 0
    )
    assert (
        len(sv.get_simplex_sampling_oversamplers(within_simplex_sampling=["random"]))
        >= 0
    )
    assert (
        len(
    ...
    assert len(sv.get_simplex_sampling_oversamplers(with_gaussian_component=True)) >= 0
    assert len(sv.get_simplex_sampling_oversamplers(with_gaussian_component=False)) >= 0
    ...
    assert (
        len(sv.generate_parameter_combinations(oversamplers, result_format="dict")) >= 0
    )

    assert (
        len(
            sv.generate_parameter_combinations(
                oversamplers, result_format="smote_variants"
            )
        )
        >= 0
    )
