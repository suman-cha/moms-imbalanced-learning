...
def test_pairwise_distances_mahalanobis():
    ...
    dmatrix_2 = pairwise_distances_mahalanobis(
        X, Y=Y, tensor=np.eye(2), optimized=False
    )
...
def test_construct_tensor():
    ...
    dissim_matrix = np.array(
        [[0.0, np.sqrt(2.0), 1.0], [np.sqrt(2.0), 0.0, 1.0], [1.0, 1.0, 0.0]]
    )
...
def test_classifierimplied_dissimmatrix():
    ...
    np.testing.assert_almost_equal(
        dissim, np.array([[0.0, 0.38, 0.16], [0.38, 0.0, 0.23], [0.16, 0.23, 0.0]])
    )
...
def test_discrete_variable_mask():
    ...
    np.testing.assert_array_equal(
        discrete_variable_mask(X, threshold=2), np.array([False, True])
    )
...
def test_n_neighbors_func():
    ...
    dist, ind = n_neighbors_func(
        X_base, X_neighbors, n_neighbors=2, return_distance=True
    )
...
def test_psd_mean():
    ...
    """
    matrices = [np.array([[1.0, 0.0], [0.0, 1.0]]), np.array([[2.0, 0.0], [0.0, 1.0]])]

    etalon = np.array([[1.5, 0.0], [0.0, 1.0]])

    np.testing.assert_array_almost_equal(psd_mean(matrices), etalon)
...
def test_metriclearningmixin():
    ...
    nn_params = {"metric_learning_method": "cov", "metric": "precomputed"}
    ...
    nn_params = {"metric": "minkowski", "metric_tensor": None}
...
def test_nn_metric_tensor():
    ...
    ind = nearestn.radius_neighbors(X_base, radius=1.2, return_distance=False)
    ...
    nearestn = NearestNeighborsWithMetricTensor(
        n_neighbors=2, metric="precomputed", metric_tensor=np.eye(2)
    )
