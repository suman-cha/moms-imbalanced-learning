...
def test_calculate_atoms():
    ...
    """
    atoms = calculate_atoms(np.array([0, 0, 0, 0, 1, 1]), np.array([0, 1, 0, 0, 1, 1]))
...
def test_calculate_label_scores():
    ...
    # no fp
    atoms = calculate_atoms(np.array([0, 1, 0, 0, 1, 1]), np.array([0, 0, 0, 0, 1, 1]))
    atoms = calculate_label_scores(atoms)

    assert atoms["lfp"] is None

    # no tp
    atoms = calculate_atoms(np.array([0, 1, 0, 0, 1, 1]), np.array([0, 0, 0, 0, 0, 0]))
    atoms = calculate_label_scores(atoms)

    assert atoms["ltp"] is None

    # no tn
    atoms = calculate_atoms(np.array([0, 1, 0, 0, 1, 1]), np.array([1, 1, 1, 1, 1, 1]))
    atoms = calculate_label_scores(atoms)
