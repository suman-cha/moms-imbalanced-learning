...
- ``AdaBoostRegressor`` implements adaptive boosting (AdaBoost.R2) for
  ...
  - ``AdaBoostClassifierOUBoost`` implements adaptive boosting (AdaBoost-SAMME) for
  classification problems.
"""
...
class BaseWeightBoosting(BaseEnsemble, metaclass=ABCMeta):
    ...
    def _boost(self, iboost, X, y, sample_weight, random_state):
        ...
        error : float
            ...
            If None then boosting has terminated early.
        """
        # print("boost BaseWeightBoosting")
        pass
...
def _samme_proba(estimator, n_classes, X):
    #print("_samme_proba: Calculate algorithm 4, step 2, equation c Multi-class AdaBoost 2009.")
...
############## Edited AdaBoostClassifier for OUBoost################################################
class AdaBoostClassifierOUBoost(ClassifierMixin, BaseWeightBoosting):
    ...
    def _validate_estimator(self):
      #  print("validate_estimator AdaBoostClassifier")
    ...
    ### X oroginal, Y original, y_org, sample_weight_original 
    def _boost(self, iboost, X, y, sample_weight, random_state, X_org, y_org, sample_weight_org):
        """Implement a single boost.
        ...
        if self.algorithm == 'SAMME.R':
            return self._boost_real(iboost, X, y, sample_weight, random_state, X_org, y_org, sample_weight_org)

        else:  # elif self.algorithm == "SAMME":
            return self._boost_discrete(iboost, X, y, sample_weight,
                                        random_state)

    def _boost_real(self, iboost, X, y, sample_weight, random_state, X_org, y_org, sample_weight_org):
        ...
        y_predict_proba = estimator.predict_proba(X_org)
        ...
        incorrect = y_predict != y_org

        # Error fraction
        estimator_error = np.mean(
            np.average(incorrect, weights = sample_weight_org, axis=0))
        ...
        y_coding = y_codes.take(classes == y_org[:, np.newaxis])
        ...
        if not iboost == self.n_estimators - 1:
            ...
            sample_weight_org *= np.exp(estimator_weight *
                                    ((sample_weight_org > 0) |
                                     (estimator_weight < 0)))
        ...
        return sample_weight, 1., estimator_error,sample_weight_org

    def _boost_discrete(self, iboost, X, y, sample_weight, random_state):
        """Implement a single boost using the SAMME discrete algorithm."""
        # print("_boost_discrete")
    ...
    def decision_function(self, X):
        ...
        """
      #  print("decision_function: Compute the decision function of ``X``")
        check_is_fitted(self)
        X = self._check_X(X)
       # print("decision_function",X.shape)
    ...
    def _compute_proba_from_decision(decision, n_classes):
        ...
        .. [1] J. Zhu, H. Zou, S. Rosset, T. Hastie, "Multi-class AdaBoost",
               2009.
        """
        # print("_compute_proba_from_decision")
